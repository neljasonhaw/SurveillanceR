...
1. Process new cases from the laboratory linelist.
    a. Identify cases that have updates in the laboratory linelist.
        i. Create a data frame of ID variable pairs - `LabSpecimenID` from the laboratory linelist and `CaseID` from the case linelist. Store results in data frame `dedup_df`.
        ii. Add names and other information in aid of manual deduplication.
            1) Store laboratory linelist information in tibble `lab_today_info`.
            2) Store case linelinse information in tibble `case_yday_info`.
            3) Merge tibble `lab_today_info` into data frame `dedup_df`. Store results in data frame `dedup_df_info`.
            4) Merge tibble `case_yday_info` into data frame `dedup_df_info`.
        iii. Generate similarity scores using Jaro-Winkler (`jw`) distance. Store results in data frame `dedup_df_jw`.
            1) Keep results with `jw` less than or equal to 0.3.
        iv. Manually inspect results in data frame `dedup_df_jw`.
            1) Subset rows that are duplicates from case linelist. These are the rows that we check later on to update the date of existing cases. Store results in data frame `dedup_df_jw_manual`.
    b. Identify new cases within the laboratory linelist.
        i. Filter positive results from laboratory linelist. Store in tibble `lab_today_pos`.
        ii. Filter out duplicates already indentified from the data frame `dedup_df_jw_manual` as those cases are not new cases.
