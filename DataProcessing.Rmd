...
* Each row in the **laboratory linelist** is a **laboratory result**, and a unique ID number of the laboratory result is automatically assigned by the reference laboratory information system using the variable `LabSpecimenID`.
  ...
  + This means that the `CaseID` variable does not *uniquely* identify the persons being tested, and we will need to deduplicate these names manually later.
* Each row in the **case linelist** is a **confirmed case**, and the unique `CaseID` number from the laboratory linelist is carried over to the case linelist. We used to process the case linelist in Microsoft Excel, so this is also a Microsoft Excel spreadsheet.
...
* `Age`: Calculated age based on date of birth encoded into the information system. For simplicity, we will not show the date of birth variable, but it is ideal that ages are calculated from date of birth and not just reporting what the age was at the time of testing
...
When we conduct data processing, it is important that we set up clear **adjudication** or validation rules. Most of the time, the validation will come outside of data processing. For example, we may instruct the reporting health facility to reencode the laboratory result in the information system. We may also review other health records and replace values manually in the information system. If we need to make modifications during data processing, we must be clear about the **adjudication rules**. We will work with a fairly simple set of rules to demonstrate this point.
...
* The variables `Name`, `Age`, `Sex`, and `Municipality` are carried over from the laboratory linelist.
  ...
  + In this example, we will work with discrepancies in the `Name` value. The name with the most complete information is retained. For example, if one value has the middle name and the other does not, then the one with the middle name is retained.
* The dates of specimen collection and result released are set up differently from the laboratory linelist. Specifically, in this case, we are primarily concerned with the first positive results (`DateSpecimenCollection_PositiveFirst` and `DateResultReleased_PositiveFirst`) and first negative result after that (`DateSpecimenCollection_NegativeFirst` and `DateResultReleased_NegativeFirst`).
    ...
    + This means that we have to restructure the values from the `DateSpecimenCollection`, `DateResultReleased`, and `Result` from the laboratory linelist to fit the columns in the linelist of cases.
* The date of symptom onset `DateOnset` is also carried over from the laboratory linelist, unless there are discrepancies with the values from a more recent laboratory result entry. 
  + For this example, our rule is that we keep the earliest recorded date of symptom onset, e.g. if the original date of symptom onset recorded was July 1, 2021, but the latest laboratory result recorded it as July 3, 2021, then there must be adjudication rules in place.
  + We will also check whether the date of symptom onset is on or before the date of specimen collection, as our hypothetical testing policy is that only symptomatic cases are going to be tested. If the date of symptom onset is after the date of specimen collection, then we replace the date of symptom onset with the date of specimen collection.
...
You may download all the files in the [Github repository here](https://github.com/neljasonhaw/SurveillanceR). Click on the green button Code then click Download ZIP. Unzip and look for the file names `fakelablinelist.csv` and `fakecaselinelist.csv`.

There is also an R script file called `DataProcessing.R`. That R script file contains all the codes in this guide. We may open that as you follow along. All of the other files are not relevant for the guide or are output files that will be generated as part of the analysis.
...
6. Save the script file every now and then. Give it a name. In the repository, this is named `DataProcessing.R`. Open that script file if you just want to run the code.

Alternatively, if you are familiar with setting up Git on RStudio, you may also set up the RStudio project by cloning the repository instead of downloading the ZIP file.

## Loading packages

We will use the following packages. We make sure the packages are installed beforehand using the `install.packages()` function.
...
We will now import both linelists. We will store the the laboratory linelist in a tibble called `lab_today`, while we will store the case linelist in a tibble called `case_yday`. [Tibbles are data frames](https://r4ds.had.co.nz/tibbles.html) that make data manipulation using the `tidyverse` package a little easier.
...
The full laboratory linelist looks like this:
...
The full case linelist looks like this
...
If we look at the data structure of both `lab_today` and `case_today` using the `str()` function in base R, we notice the dates are read as a calendar date and time object (`POSIXct`). 
...
```{r datesample, warning = FALSE, message = FALSE}
...
# Deduplicating new cases

We will now deduplicate new cases from the lab linelist. We need to conduct two deduplication activities:

* First, we will need to check if any of the results from `lab_today` have already been added as cases in `case_yday`, and whether these recent laboratory results are additional positive results or negative results.
* Second, we will need to deduplicate new cases within `lab_today`. There might be cases whose names appear twice in `lab_linelist` but are new cases that are not yet found on `case_yday`

Because our hypothetical laboratory information system can only detect exact matches of names for the generation of the `CaseID`, we will need to use fuzzy matching to manually deduplicate names.


## Deduplication between recent laboratory results and previous cases

### Setting up the deduplication pairs
We will create a tibble (`dedup_df`) of `LabSpecimenID` of the tibble `lab_today` paired with all `CaseID` in the tibble `case_yday`. Recall that these ID variables uniquely identify each row. Therefore, the tibble `dedup_df` represents all pairs of names that we will instruct R later on to measure a similarity score. To retrieve the ID columns for both tibbles `lab_today` and `case_today`, we will use `dplyr`'s `select()` function then use `dplyr`'s `pull()` function to save as a vector (`case_yday_id` and `lab_today_id`), then finally use the base R function `expand.grid()` to generate a data frame of all ID pairs. To make our code readable, we use `dplyr`'s pipe operator (`%>%`) instead of nesting parentheses.

```{r dedup_df, warning = FALSE, message = FALSE}
# Retrieve ID columns and save as vector
lab_today_id <- lab_today %>% select(LabSpecimenID) %>% pull()
case_yday_id <- case_yday %>% select(CaseID) %>% pull()

# Generate all pairwise matches of IDs
# By default, expand.grid treats the string values as factors, 
# so we will have to instruct R not to do that
dedup_df <- expand.grid(lab_today_id, case_yday_id, stringsAsFactors = FALSE)
...
### Add names and other information for deduplication
We will now add more information to the names to aid us in decision making on whether or not the pairs are duplicates. In our linelist, the age, sex, and municipality all help in making that decision. We will also add the ID variables, the date of symptom onset, and laboratory test result. This involves multiple steps. We will first retrieve the relevant columns from `lab_today` and `case_yday` using `dplyr`'s `select()` function. We will then store the results in the tibbles `lab_today_info` and `case_yday_info`.
...
Then we merge the data from the tibble `lab_today_info` into the data frame `dedup_df` and store into a new data frame called `dedup_df_info` using the base R function `merge()`. We will also arrange the columns as we go along with the merging using `dplyr`'s `relocate()` function.
...
At this point, the data frame `dedup_df_info` has the relevant columns from the tibble `lab_today_info` merged with the column `lab_today_id` from the data frame `dedup_df`, and we have placed the column `case_yday_id` at the end. Now, we will execute similar steps to merge the data from `case_yday_info`.
...
# Visually this will remind us that lab_today_info are columns with suffix .x
# and case_yday_info are columns with suffix .y
dedup_df_info <- dedup_df_info %>% relocate(case_yday_id, .before = Name.y)
```

### Generating similarity scores
We will now run the `tidy_stringdist()` function to generate similarity scores of all our pairwise names. You may read more about the different methods under this command by reading the documentation (Type `?tidy_stringdist-metrics` on the Console). If we do not specify the method, the command generates a score using all available methods (which can be time-consuming), so in this example, we will just use one - the Jaro-Winkler distance (`jw`), which provides similarity scores on a scale of 0 to 1, where 0 is an exact match (the lower the score, the likelier it is a duplicate). We will store the results in the data frame `dedup_df_jw` and retain pairs with a score of 0.3 and below for further inspection using `dplyr`'s `filter()` function, then sort by `jw` using `dplyr`'s `arrange()` function. Feel free to experiment with what method and cutoff works.

```{r dedup_tidystringdist, warning = FALSE, message = FALSE}
...
Now with the data frame `dedup_df_jw`, we will manually select rows that are duplicates by looking at the entire data frame. We will type `View(dedup_df_jw)` on the Console to view it as a separate tab on the Script editor. Take note of the rows that we will mark as duplicates.
...
The tibble `dedup_df_jw_manual` now contains laboratory results of cases already existing in the case linelist, and we will need to update their information later on with this tibble. The next step deduplicates names among new cases **within** the laboratory linelist.


## Deduplicating new cases within the laboratory linelist

### Filtering positive results
We will first filter only positive results from the laboratory linelist (`lab_today`) because these contain all the new cases and current cases with subsequent positive results. We will save the filtered data in a tibble called `lab_today_pos`.
...
### Setting up the deduplication pairs
We will now create a tibble (`dedup_new_df`) of `LabspecimenID` in the tibble `lab_today_pos_nodup` paired with all other `LabSpecimenID` in the same tibble. We will use the `expand.grid()` function again. To save some space, we automatically delete the pairs that are equal to one another using the `filter()` function.
...
We will now add more information to the names to aid us in decision making on whether or not the pairs are duplicates, and to decide which among the duplicates do we retain. We will use age, sex, and municipality, date of symptom onset, date of specimen collection. and date of result released to make that decision. We will also add the `CaseID` variable. We will store the filtered columns in the tibble `lab_today_pos_nodup_info`.
...
We will now run the `tidy_stringdist()` function to generate similarity scores of all our pairwsise names. We will use the `jw` method and retain scores 0.3 and below for further inspection. We will store the results in a new data frame called `dedup_new_df_jw`. We will also sort the results by `jw` and `Name.x` (the first name column) in aid of manual inspection.
...
### Manually selecting duplicates
Now with the data frame `dedup_new_df_jw`, we will manually select rows that are duplicates by looking at the entire data frame. We will type `View(dedup_new_df_jw)` on the Console to view it as a separate tab on the Script editor. Take note of the rows that we will choose to retain.
...
There are no rows to exclude as all the rows are duplicates of one another. Rows 1 and 2, 3 and 4, and 5 and 6 are duplicates of one another. We save the results in a new data frame `dedup_new_df_jw_manual`. We add a new column called `DuplicateID` which will manually tag rows as duplicates of one another.
...
# Row 1 and 2 will have DuplicateID 1
# Row 3 and 4 will have DuplicateID 2
# Row 5 and 6 will have DuplicateID 3
...
Now we will decide which information to use from each columns among the new cases in `dedup_new_df_jw_manual`. Recall the simple adjudication rules:
...
Then, we retrieve all the columns we need from the tibble `lab_today` and merge them with the `lab_today_id` in the data frame `dedup_df_jw_IDpairs`. We will store the results in a tibble called `dedup_df_jw_allinfo`
...
### Preparing to add to case linelist
Now we transform the the data frame `dedup_df_jw_allinfo` into the columns of the tibble `case_yday` so that it will be easier for us to update case information. We store the new tibble as `case_yday_newinfo`. 

* We carry over the values of `CaseID`, `Name`, `Age`, `Sex`, `Municipality`, and `DateOnset`
* The values in `DateSpecimenCollection` and `DateResultReleased` are assigned to `DateSpecimenCollection_PositiveFirst` and `DateResultReleased_PositiveFirst` if the result is positive, and `DateSpecimenCollection_NegativeFirst` and `DateResultReleased_NegativeFirst` if the result is negative. We will use the `dplyr`'s `mutate()` and `case_when()` functions for this. Note that the pipe operator (`%>%`) can be used to execute multiple instructions at once.
* We will then remove the columns `lab_today_id`, `CaseID`, `DateSpecimenCollection`, `DateResultReleased` and `Result` since we will not need them anymore after this. `CaseID` in this data frame pertain to the autogenerated `CaseID` from the laboratory linelist. We need the column `case_yday_id` because that is the ID variable that will allow us to link them to the tibble `case_yday`. then we will rename `case_yday_id` to `CaseID`. 
...
Now, we will add rows to the tibble `case_yday_newinfo` that contain information from the case linelist. We will save these results in a tibble called `case_yday_oldinfo`. We will then combine the two tibbles into a new tibble called `case_yday_recon`. In this tibble, we will implement the adjudication rules and save them in a new tibble called `case_yday_update`
...
Finally, we will check whether the date of symptom onset is on or before the date of specimen collection of the first positive test result. We mentioned earlier that when we find dates of symptom onset that are greater than the date of specimen collection, we replace the date of symptom onset with the date of specimen collection.
